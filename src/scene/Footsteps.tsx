 ...
 * travel — that expands as a fading ring. It's the "I am making noise"
...
export function Footsteps({ entity }: { entity: Entity }) {
  ...
  const ripples = useRef<Array<{ x: number; z: number; t: number }>>([]);
  ...
  useFrame((_, delta) => {
    ...
    if (speed > 0.5) {
      ...
      if (traveled.current >= STRIDE) {
        ...
        ripples.current.push({ x: pos.x + ox, z: pos.z + oz, t: 0 });
    ...
    for (const r of ripples.current) {
      ...
      dummy.scale.setScalar(0.2 + k * 1.05);
      dummy.updateMatrix();
      m.setMatrixAt(i, dummy.matrix);
      // Additive blending: fading the color to black IS the fade-out.
      m.setColorAt(i, color.set('#8fa3c8').multiplyScalar(0.5 * (1 - k)));
      i++;
    }
    m.count = i;
